...
// Fungsi Middleware CORS ditaruh di sini agar praktis
func CORSMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Writer.Header().Set("Access-Control-Allow-Origin", "*") // Izinkan Flutter Web Anda mengakses API
		c.Writer.Header().Set("Access-Control-Allow-Credentials", "true")
		c.Writer.Header().Set("Access-Control-Allow-Headers", "Content-Type, Content-Length, Accept-Encoding, X-CSRF-Token, Authorization, accept, origin, Cache-Control, X-Requested-With")
		c.Writer.Header().Set("Access-Control-Allow-Methods", "POST, OPTIONS, GET, PUT, DELETE")

		// JIKA METHOD ADALAH OPTIONS (PREFLIGHT REQUEST FROM CHROME), LANGSUNG RESPON 204
...
func Setup(db *gorm.DB, rdb *redis.Client, firebaseApp *firebase.App, cfg *config.Config) *gin.Engine {
	...
	// --- PASANG MIDDLEWARE CORS DI SINI ---
